...
class TestRMSDCalculators(unittest.TestCase):
    ...
    def test_serial_omp_pairwise(self):
        ...
        """
        expected_rmsd_data = [0.22677106513739653, 0.44598234794295144, 0.37817804816455303]
        calculator = pyRMSD.RMSDCalculator.RMSDCalculator(self.coordsets_mini, "OMP_CALCULATOR")
        rmsd = []
        for i in range(len(self.coordsets_mini)):
            for j in range(i+1, len(self.coordsets_mini)):
                rmsd.append(calculator.pairwise(i, j))
    ...
    @unittest.skipIf(not "THEOBALD_CUDA_CALCULATOR" in pyRMSD.RMSDCalculator.availableCalculators(),"CUDA calculator not available")
    def test_theobald_cuda_pairwise(self):
        """
        Calculates all matrix elements one by one with the pairwise operation.
        """
        expected_rmsd_data = [0.22677106513739653, 0.44598234794295144, 0.37817804816455303]
        calculator = pyRMSD.RMSDCalculator.RMSDCalculator(self.coordsets_mini, "THEOBALD_CUDA_CALCULATOR")
        rmsd = []
        for i in range(len(self.coordsets_mini)):
            for j in range(i+1, len(self.coordsets_mini)):
                rmsd.append(calculator.pairwise(i, j))
    ...
    def test_serial(self):
        ...
        for conf_num in self.expected_rmsd:
            calculator = pyRMSD.RMSDCalculator.RMSDCalculator(self.coordsets, "SERIAL_CALCULATOR")
    ...
    def test_OpenMP(self):
        ...
        """
        for conf_num in self.expected_rmsd:
            calculator = pyRMSD.RMSDCalculator.RMSDCalculator(self.coordsets, "OMP_CALCULATOR")
            rmsd = calculator.oneVsFollowing(conf_num)
            numpy.testing.assert_array_almost_equal(rmsd, self.expected_rmsd[conf_num],8)
    ...
    def test_theobald_serial(self):
        ...
        """
        for conf_num in self.expected_rmsd:
            calculator = pyRMSD.RMSDCalculator.RMSDCalculator(self.coordsets, "THEOBALD_SERIAL_CALCULATOR")
            rmsd = calculator.oneVsFollowing(conf_num)
            numpy.testing.assert_array_almost_equal(rmsd, self.expected_rmsd[conf_num],8)
    ...
    def test_theobald_OpenMP(self):
        ...
        """
        for conf_num in self.expected_rmsd:
            calculator = pyRMSD.RMSDCalculator.RMSDCalculator(self.coordsets, "THEOBALD_SERIAL_OMP_CALCULATOR")
            rmsd = calculator.oneVsFollowing(conf_num)
            numpy.testing.assert_array_almost_equal(rmsd, self.expected_rmsd[conf_num],8)
    ...
    def test_theobald_CUDA(self):
        ...
        """
        for conf_num in self.expected_rmsd:
            calculator = pyRMSD.RMSDCalculator.RMSDCalculator(self.coordsets, "THEOBALD_CUDA_CALCULATOR")
            rmsd = calculator.oneVsFollowing(conf_num)
