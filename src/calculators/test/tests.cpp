...
void test_rmsd_calculation_fit_with_symmetry(RMSDCalculatorType type){
	print_test_tittle(__FUNCTION__);
	vector<double> native_plus_coords_lig;
	vector<int> trajectory_with_native_lig_size;
	vector <double> expected_rmsds, calculated_rmsds;


	// Load rmsds
	load_vector(expected_rmsds, "data/Symmetry/OneVsAllFit/minimum.rmsd");
	...
	load_pdb_coords(native_0_plus_coords_lig,
				trajectory_with_native_lig_size,
				"data/Symmetry/OneVsAllFit/Native_0_ligand.pdb.aligned_BEN.coords");

	load_pdb_coords(native_1_plus_coords_lig,
				trajectory_with_native_lig_size,
				"data/Symmetry/OneVsAllFit/Native_1_ligand.pdb.aligned_BEN.coords");

	load_pdb_coords(native_2_plus_coords_lig,
				trajectory_with_native_lig_size,
				"data/Symmetry/OneVsAllFit/Native_2_ligand.pdb.aligned_BEN.coords");

	load_pdb_coords(native_3_plus_coords_lig,
				trajectory_with_native_lig_size,
				"data/Symmetry/OneVsAllFit/Native_3_ligand.pdb.aligned_BEN.coords");

	calculated_rmsds_0.resize(trajectory_with_native_lig_size[0]-1,0);
	calculated_rmsds_1.resize(trajectory_with_native_lig_size[0]-1,0);
	calculated_rmsds_2.resize(trajectory_with_native_lig_size[0]-1,0);
	calculated_rmsds_3.resize(trajectory_with_native_lig_size[0]-1,0);

	load_pdb_coords(native_plus_coords_lig,
			trajectory_with_native_lig_size,
			"data/Symmetry/OneVsAllFit/Native_3_ligand.pdb.aligned_BEN.coords");

	print_vector<int>("SIZE: ",TOPOINTER(trajectory_with_native_lig_size), 3, 1);

	RMSDCalculator* calculator = RMSDCalculatorFactory::createCalculator(
										type,
										trajectory_with_native_lig_size[0],
										trajectory_with_native_lig_size[1],
										TOPOINTER(native_0_plus_coords_lig));

	calculator->oneVsFollowing(0, TOPOINTER(calculated_rmsds_0));
	print_vector<double>("Calculated with native 0: ", TOPOINTER(calculated_rmsds_0), calculated_rmsds_0.size(),12);
	delete calculator;
	...
	calculator->oneVsFollowing(0, TOPOINTER(calculated_rmsds_1));
	print_vector<double>("Calculated with native 1: ", TOPOINTER(calculated_rmsds_1), calculated_rmsds_1.size(),12);
	delete calculator;
	...
	calculator->oneVsFollowing(0, TOPOINTER(calculated_rmsds_2));
	print_vector<double>("Calculated with native 2: ", TOPOINTER(calculated_rmsds_2), calculated_rmsds_2.size(),12);
	delete calculator;
	...
	calculator->oneVsFollowing(0, TOPOINTER(calculated_rmsds_3));
	print_vector<double>("Calculated with native 3: ", TOPOINTER(calculated_rmsds_3), calculated_rmsds_3.size(),12);
	delete calculator;
	...
	calculated_rmsds.resize(trajectory_with_native_lig_size[0]-1,0);
	calculator = RMSDCalculatorFactory::createCalculator(
									type,
									trajectory_with_native_lig_size[0],
									trajectory_with_native_lig_size[1],
									TOPOINTER(native_plus_coords_lig),
									0,
									NULL,
									&symm_groups);
	calculator->oneVsFollowing(0, TOPOINTER(calculated_rmsds));
	...
	print_vector<double>("expected RMSD!: ", TOPOINTER(expected_rmsds), expected_rmsds.size(),12);
	// The result has to be similar (at least in the same order) than the one got with prody
	compareVectors("\tResults are within the safety range: ",
							TOPOINTER(expected_rmsds),
							TOPOINTER(calculated_rmsds),
							expected_rmsds.size(),
							1e-12); // Qualitative check

	print_vector<int>("coords_size: ", TOPOINTER(trajectory_with_native_lig_size),trajectory_with_native_lig_size.size(), 1 );
}
